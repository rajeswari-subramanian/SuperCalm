...
function Week() {
    ...
    return (
        <>
            <div style={{ display: "flex", fontWeight: "bolder" }}>{["Week1", "Week2", "Week3", "Week4"].map((item, i) => (
                <WeekButtons onClick={() => handleWeek(item)} key={i} type="button" class="btn btn-lg">{item}</WeekButtons>
            ))}</div>
            <div style={{ position: "absolute", fontSize: "40px", top: "100px", left: "250px" }}>
                <div style={{ display: "flex", flexDirection: "column" }}>
                    <i style={{ color: 'green' }} class="far fa-laugh-squint"></i><i style={{ color: 'green' }} class="far fa-smile"></i><i style={{ color: 'yellow' }} class="far fa-meh"></i><i style={{ color: 'red' }} class="far fa-frown"></i><i style={{ color: 'red' }} class="far fa-sad-cry"></i></div></div>
